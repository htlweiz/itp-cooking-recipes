...
function getNutritionalInfo() {
  const totalCalories = recipe.value.ingredients.reduce((sum, ing) => sum + ing.calories, 0);
  const totalProtein = recipe.value.ingredients.reduce((sum, ing) => sum + ing.protein, 0) + 'g';
  const totalCarbs = recipe.value.ingredients.reduce((sum, ing) => sum + ing.carbs, 0) + 'g';
  const totalFat = recipe.value.ingredients.reduce((sum, ing) => sum + ing.fat, 0) + 'g';
